...
contract USDA is Initializable, PausableUpgradeable, UFragments, IUSDA, ExponentialNoError, Roles {
    ...
    modifier paysInterest() {
        for (uint256 i; i < _vaultControllers.length();) {
    ...
    /// @param _susd_amount amount of sUSD to deposit
    function deposit(uint256 _susd_amount) external override {
        _deposit(_susd_amount, _msgSender());
    }

    function depositTo(uint256 _susd_amount, address _target) external override {
        _deposit(_susd_amount, _target);
    }

    function _deposit(uint256 _susd_amount, address _target) internal paysInterest whenNotPaused {
        // scale the _susd_amount to the usda decimal amount, aka 1e18. since susd is 6 decimals, we multiply by 1e12
        uint256 _amount = _susd_amount * 1e12;
        if (_amount == 0) revert USDA_ZeroAmount();
        // Account for the susd received
        reserveAmount += _susd_amount;
        if (!reserve.transferFrom(_msgSender(), address(this), _susd_amount)) revert USDA_TransferFailed();
        ...
        emit Deposit(_target, _amount);
    }

    /// @notice withdraw sUSD by burning USDA
    /// caller should obtain 1 sUSD for every 1e12 USDA
    /// @param _susd_amount amount of sUSD to withdraw
    function withdraw(uint256 _susd_amount) external override {
        _withdraw(_susd_amount, _msgSender());
    }

    ///@notice withdraw sUSD to a specific address by burning USDA from the caller
    /// _target should obtain 1 sUSD for every 1e12 USDA burned from the caller
    /// @param _susd_amount amount of sUSD to withdraw
    /// @param _target address to receive the sUSD
    function withdrawTo(uint256 _susd_amount, address _target) external override {
        _withdraw(_susd_amount, _target);
    }

    ///@notice business logic to withdraw sUSD and burn USDA from the caller
    function _withdraw(uint256 _susd_amount, address _target) internal paysInterest whenNotPaused {
        // scale the _susd_amount to the USDA decimal amount, aka 1e18
        uint256 _amount = _susd_amount * 1e12;
        // check balances all around
        if (_amount == 0) revert USDA_ZeroAmount();
        if (_amount > this.balanceOf(_msgSender())) revert USDA_InsufficientFunds();
        // Account for the susd withdrawn
        reserveAmount -= _susd_amount;
        // ensure transfer success
        if (!reserve.transfer(_target, _susd_amount)) revert USDA_TransferFailed();
        // modify the gonbalances of the sender, subtracting the amount of gons, therefore amount*gonsperfragment
        _gonBalances[_msgSender()] = _gonBalances[_msgSender()] - _amount * _gonsPerFragment;
    ...
    /// @notice withdraw sUSD by burning USDA
    /// @param _target should obtain 1 sUSD for every 1e12 USDA burned from caller
    ...
    function _withdrawAll(address _target) internal paysInterest whenNotPaused {
        ...
        uint256 _susd_amount = (this.balanceOf(_msgSender())) / 1e12;
        //user's USDA value is more than reserve
        if (_susd_amount > reserveAmount) _susd_amount = reserveAmount;
        uint256 _amount = _susd_amount * 1e12;
        // Account for the susd withdrawn
        reserveAmount -= _susd_amount;
        if (!reserve.transfer(_target, _susd_amount)) revert USDA_TransferFailed();
    ...
    /// @param _susd_amount the amount of USDA to mint, denominated in sUSD
    function mint(uint256 _susd_amount) external override paysInterest onlyOwner {
        if (_susd_amount == 0) revert USDA_ZeroAmount();
        uint256 _amount = _susd_amount * 1e12;
    ...
    /// @param _susd_amount the amount of USDA to burn, denominated in sUSD
    function burn(uint256 _susd_amount) external override paysInterest onlyOwner {
        if (_susd_amount == 0) revert USDA_ZeroAmount();
        uint256 _amount = _susd_amount * 1e12;
        // see comments in the deposit function for an explaination of this math
        _gonBalances[_msgSender()] = _gonBalances[_msgSender()] - _amount * _gonsPerFragment;
    ...
    /// @param _susd_amount the amount of sUSD to donate
    function donate(uint256 _susd_amount) external override paysInterest whenNotPaused {
        if (_susd_amount == 0) revert USDA_ZeroAmount();
        uint256 _amount = _susd_amount * 1e12;
        // Account for the susd received
        reserveAmount += _susd_amount;
    ...
    /// @param _susd_amount the amount of sUSD to send
    function vaultControllerTransfer(address _target, uint256 _susd_amount) external override onlyVaultController {
        // Account for the susd withdrawn
        reserveAmount -= _susd_amount;
        // ensure transfer success
        if (!reserve.transfer(_target, _susd_amount)) revert USDA_TransferFailed();
    ...
    /// @return _e18reserve_ratio USDA reserve ratio
    function reserveRatio() external view override returns (uint192 _e18reserve_ratio) {
        _e18reserve_ratio = safeu192(((reserve.balanceOf(address(this)) * expScale) / _totalSupply) * 1e12);
    }
