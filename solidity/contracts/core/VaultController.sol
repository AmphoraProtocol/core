    ...
    OwnableUpgradeable
{
    ...
    function borrowUSDA(uint96 id, uint192 amount) external override {
    ...
    function borrowUSDAto(uint96 id, uint192 amount, address target) external override {
    ...
    /// @param target address to receive borrowed USDA
    /// @dev pays interest
    function _borrowUSDA(uint96 id, uint192 amount, address target) internal paysInterest whenNotPaused {
        // grab the vault by id if part of our system. revert if not
        IVault vault = getVault(id);
    ...
    /// @param id vault to borrow against
    /// @param susd_amount amount of sUSD to borrow
    /// @param target address to receive borrowed sUSD
    function borrowsUSDto(uint96 id, uint192 susd_amount, address target)
        external
        override
        paysInterest
        whenNotPaused
    {
        ...
        _usda.vaultControllerTransfer(target, susd_amount);
    }
    ...
    function repayAllUSDA(uint96 id) external override paysInterest whenNotPaused {
        ...
        uint256 usda_liability = uint256(safeu192(truncate(_interest.factor * baseLiability)));
    ...
    function liquidateVault(uint96 id, address asset_address, uint256 tokens_to_liquidate)
        external
        override
        paysInterest
        whenNotPaused
        returns (uint256)
    {
        ...
        if (tokenAmount != 0) tokens_to_liquidate = tokenAmount;
        ...
        _totalBaseLiability = _totalBaseLiability - safeu192((usda_to_repurchase * 1e18) / _interest.factor);
    ...
    function _liquidationMath(uint96 id, address asset_address, uint256 tokens_to_liquidate)
        internal
        view
        returns (uint256, uint256)
    {
        ...
        if (tokens_to_liquidate > max_tokens_to_liquidate) tokens_to_liquidate = max_tokens_to_liquidate;
    ...
    /// @param id id of vault
    /// @return amount of USDA the vault can borrow
    ...
    function get_vault_borrowing_power(IVault vault) private view returns (uint192) {
        ...
        for (uint192 i = 1; i <= _tokensRegistered; ++i) {
            ...
            if (_tokenId_tokenLTV[i] == 0) continue;
    ...
    function vaultSummaries(uint96 start, uint96 stop) public view override returns (VaultSummary[] memory) {
        ...
        for (uint96 i = start; i <= stop; i++) {
            ...
            summaries[i - start] =
                VaultSummary(i, this.vaultBorrowingPower(i), this.vaultLiability(i), _enabledTokens, tokenBalances);
        }
        return summaries;
    }
}
