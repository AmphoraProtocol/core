...
import {VotingVault} from '@contracts/core/VotingVault.sol';
    ...
    function mintVault(uint96 id) public returns (address) {
        if (_vaultId_votingVaultAddress[id] == address(0)) {
            ...
            if (vault_address != address(0)) {
                ...
                address voting_vault_address =
                    address(new VotingVault(id, vault_address, address(_vaultController), address(this)));
