 ...
 * @dev USDA uses the uFragments concept from the Ideal Money project to play interest
 ...
 */
contract UFragments is CustomInitializable, OwnableUpgradeable, ERC20Detailed {
    ...
    function __UFragments_init(string memory name, string memory symbol) public initializer2 {
        __Ownable_init();
    ...
    function DOMAIN_SEPARATOR() public view returns (bytes32) {
        ...
        return keccak256(
            abi.encode(
                EIP712_DOMAIN, keccak256(bytes(name())), keccak256(bytes(EIP712_REVISION)), chainId, address(this)
            )
        );
    }
