...
contract E2ECap is CommonE2EBase {
  uint256 public aaveDepositAmount = 500 ether;
  uint256 public dydxDepositAmount = 5 ether;
  ...
  function _depositCapped(address _account, address _underlying, address _capped, uint256 _amountToDeposit, address _vaultAddress) internal {
    ...
    IERC20(_capped).approve(_vaultAddress, _amountToDeposit);
    IVault(_vaultAddress).depositERC20(_capped, _amountToDeposit);
    vm.stopPrank();
  }
  ...
  function testSetCap() public {
    ...
    assert(aaveCappedToken.cap() == AAVE_CAP);
  }
