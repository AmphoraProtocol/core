...
contract E2EUSDA is CommonE2EBase {
    ...
    function testDepositAndInterestWithStartingBalance() public {
        ...
        emit Deposit(address(dave), _susdAmount * 1e12);

        vm.prank(dave);
        usdaToken.deposit(_susdAmount);

        assertEq(susd.balanceOf(dave), daveSUSD - _susdAmount);
        ...
        assertGt(usdaToken.balanceOf(dave), _daveUSDABalance + _susdAmount);
    }
    ...
    function testWithdrawSUSD() public {
        ...
        vm.expectRevert('Pausable: paused');
        vm.prank(dave);
        usdaToken.withdraw(_susdAmount);
        ...
        assertEq(_susdBefore, daveSUSD - _susdAmount);
