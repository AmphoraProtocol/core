...
contract E2EVaultController is CommonE2EBase {
     ...
     */
    function payInterestMath(uint192 _interestFactor) internal view returns (uint192 _newInterestFactor) {
    ...
    function calculateAccountLiability(
     ...
     */
    function calculateBalance(uint192 _interestFactor, address _user) internal view returns (uint256 _balance) {
        ...
        uint192 _calculatedInterestFactor = payInterestMath(_interestFactor);
    ...
    function testBorrow() public {
        ...
        uint256 _expectedInterestFactor = payInterestMath(_interestFactor);

        uint256 _liability = calculateAccountLiability(borrowAmount, _interestFactor, _interestFactor);
    ...
    function testLiabilityAfterAWeek() public {
        ...
        uint256 _liability = calculateAccountLiability(borrowAmount, _interestFactor, _initInterestFactor);
    ...
    function testInterestGeneration() public {
        ...
        uint256 _expectedBalance = calculateBalance(_interestFactor, dave);
    ...
    function testPartialRepay() public {
        ...
        uint192 _interestFactor = vaultController.interestFactor();
        uint256 _expectedBalance = calculateBalance(_interestFactor, bob);
    ...
    function testCompletelyRepayVault() public {
        ...
        vm.prank(bob);
        uint256 _liability = bobVault.baseLiability();
        uint192 _expectedInterestFactor = payInterestMath(_interestFactor);
        uint256 _expectedUSDALiability = (_expectedInterestFactor * _liability) / 1 ether;
        uint256 _expectedBalanceWithInterest = calculateBalance(_expectedInterestFactor, bob);
