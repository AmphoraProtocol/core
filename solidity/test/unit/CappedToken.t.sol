...
abstract contract UnitCappedTokenBase is DSTestPlus {
  IERC20 public underlying = IERC20(mockContract(newAddress(), 'underlying'));
  CappedToken public cappedToken;

  uint8 public constant UNDERLYING_DECIMALS = 6;

  string public constant CAPPED_NAME = 'Underlying Capped Token';
  string public constant CAPPED_SYMBOL = 'cUND';

  address public alice = newAddress();

  function setUp() public {
    // Mock decimals call
    vm.mockCall(address(underlying), abi.encodeWithSelector(IERC20.decimals.selector), abi.encode(UNDERLYING_DECIMALS));
    ...
    cappedToken.initialize(CAPPED_NAME, CAPPED_SYMBOL, address(underlying));
  }
}

contract UnitCappedTokenInitialValues is UnitCappedTokenBase {
  function testERC20Metadata() public view {
    assert(keccak256(abi.encodePacked(CAPPED_NAME)) == keccak256(abi.encodePacked(cappedToken.name())));
    assert(keccak256(abi.encodePacked(CAPPED_SYMBOL)) == keccak256(abi.encodePacked(cappedToken.symbol())));
    assert(18 == cappedToken.decimals());
  }

  function testGetCap() public view {
    assert(cappedToken.cap() == 0);
  }
...
contract UnitCappedTokenDeposit is UnitCappedTokenBase {
  ...
  function testDepositMintCappedToken(uint256 _amount) public {
    ...
    // deposit
    vm.prank(alice);
    vm.expectCall(address(underlying), abi.encodeCall(underlying.transferFrom, (alice, address(cappedToken), _amount)));
    cappedToken.deposit(_amount, alice);
