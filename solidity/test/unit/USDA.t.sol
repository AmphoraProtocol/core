...
import '@contracts/core/USDA.sol';
import '@interfaces/core/IVaultController.sol';
...
abstract contract Base is DSTestPlus {
    ...
    address _vaultController = mockContract(newAddress(), 'mockVaultController');
    address _vaultController2 = mockContract(newAddress(), 'mockVaultController2');
...
contract UnitUSDAWithdraw is Base {
    uint256 _depositAmount = 100 * 1e6;
...
contract UnitUSDADonate is Base {
    uint256 _donateAmount = 10000 * 1e6;
    address _otherUser = newAddress();
    ...
    function testAddsToUserBalances(uint256 _amount) public {
        ...
        assertApproxEqAbs(
            _balanceOtherUserBefore + (_amount * 1e12 - _amountToZeroAddress) / 2, _balanceOtherUserAfter, DELTA
        );

        assertApproxEqAbs(_balanceAdminBefore + (_amount * 1e12 - _amountToZeroAddress) / 2, _balanceAdminAfter, DELTA);
    }
}

contract UnitUSDARecoverDust is Base {
    uint256 _amountToRecover = 100 * 1e6;
    uint256 _depositAmount = 100000 * 1e6;
...
contract UnitUSDAVaultControllerMint is Base {
    uint256 _amountToMint = 100 * 1e18;
...
contract UnitUSDAVaultControllerTransfer is Base {
    uint256 _amountToTransfer = 100 * 1e18;
